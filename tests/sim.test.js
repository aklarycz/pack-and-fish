...
test('latched fish that dies becomes a bubble and scores', () => {
  const s = started();
  const hookX = 200;
  s.fish.push({ type: 'plotka', x: hookX, y: WORLD.hookStartY, hp: 6, hpMax: 6, window: 2.0, windowLeft: 0, state: 'aggro', dir: 1, bubbleY: 0 });
  let safety = 0;
  while (s.stunned === 0 && safety++ < 200) stepDescent(s, hookX, 0.1, () => 0.5);
...
test('descent runs many steps without throwing and accrues depth', () => {
  ...
  for (let i = 0; i < 400; i++) {
    ...
    stepDescent(s, x, 0.05, () => ((i * 0.131) % 1));
  }
